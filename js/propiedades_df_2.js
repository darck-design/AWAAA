var diagrama_inicial1 = `
{ 
	"class": "GraphLinksModel",
  	"linkFromPortIdProperty": "frompid",
  	"linkToPortIdProperty": "topid",
  	"nodeDataArray": [ 
		{"category":"Start", "text":"Inicializar", "key":-1, "loc":"75.63777923583984 27.821252512377363"},`;

var diagrama_inicial2 = `
		{"category":"End", "text":"Finalizar", "key":-3, "loc":"75.63777923583993 205.64250502475483"}
	],
  	"linkDataArray": [ 
		{"from":-1, "to":-2, "frompid":"OUT", "topid":"IN"},
		{"from":-2, "to":-3, "frompid":"OUT", "topid":"T"}
	]
}`;

var diagrama_inicial = "";

function crear_diagrama_inicial(){
    var variables_valori = '{"category":"Calcular", "text":"';

    for(i in a_ids){
        variables_valori = variables_valori + a_ids[i] + '=' + a_valor_ini[i] + '\\n';
    }
    variables_valori = variables_valori + '", "key":-2, "loc":"75.63777923583984 120.64250502475471"},';

    this.diagrama_inicial = diagrama_inicial1 + variables_valori + diagrama_inicial2;
}
    ...
    function load() {
      crear_diagrama_inicial();
      document.getElementById("mySavedModel").value = diagrama_inicial;
    ...
    function init() {
      ...
      function puerto_salida_condicion(name, align, spot, etiqueta, leftside) {
        var horizontal = align.equals(go.Spot.Top) || align.equals(go.Spot.Bottom);
        // the port is basically just a transparent rectangle that stretches along the side of the node,
        // and becomes colored when the mouse passes over it
        var port = $(go.Shape,
          {
            ...
            toSpot: spot,  // declare where links may connect at this port
            fromLinkable: true,  // declare whether the user may draw links to here
            cursor: "pointer",  // show a different cursor to indicate potential link point
            mouseEnter: function(e, port) {  // the PORT argument will be this Shape
              if (!e.diagram.isReadOnly) port.fill = "rgba(255,0,255,0.5)";
            },
          ...
          var lab = $(go.TextBlock, etiqueta,  // the name of the port
            { font: "10pt sans-serif" });
  
          var panel = $(go.Panel, "Vertical");

          if (leftside) {
            lab.margin = new go.Margin(0, 30, 0, 0);
            //panel.add(lab);
            //panel.add(port);
          }else{
            lab.margin = new go.Margin(0, 0, 0, 30);
            //panel.add(port);
            //panel.add(lab);
          }
          panel.add(lab);
      ...
      */
      ...
      myDiagram.nodeTemplateMap.add("Condicion",
        $(go.Node, "Table", nodeStyle(),
          $(go.Panel, "Auto",
            $(go.Shape, "Diamond",
              { minSize: new go.Size(40, 40), fill: "#e3da0b", strokeWidth: 0 }),
            $(go.TextBlock, textStyle(),
              {
                margin: 8,
                maxSize: new go.Size(160, NaN),
          ...
          // puertos para las entradas
          puerto_entrada("IN", go.Spot.Top, go.Spot.TopSide, false, true),
          puerto_salida_condicion("OUT1", go.Spot.Left, go.Spot.LeftSide, "No", true),
          puerto_salida_condicion("OUT2", go.Spot.Right, go.Spot.RightSide, "Si", false)
        ));
